...
We have one dataset with 24 samples, with 2 REPS2 and 3 CHD9. 
...
load("../results/episignatures/2021-12-20/group_features.Rdata")
```
...
The number of CpGs with episignatures is higher than 200 for each case.
...
pheatmap(mat_reps2, scale = "none", 
		annotation_col  = data.frame(colData(gset[gset$Grupo2 %in% c("REPS2", "Control"), ])[, c("Slide", "Familia", "Grupo2")]),
...
pheatmap(mat_chd9, scale = "none", 
		annotation_col  = data.frame(colData(gset[gset$Grupo2 %in% c("CHD9", "Control"), ])[, c("Slide", "Familia", "Grupo2")]),
		annotation_colors =  col_colors, 
		show_rownames = FALSE)
...
pheatmap(mat_caso, scale = "none", 
		annotation_col  = data.frame(colData(gset[gset$Grupo2 %in% c("Caso", "Portador", "Control"), ])[, c("Slide", "Familia", "Grupo2")]),
		annotation_colors =  col_colors, 
		show_rownames = FALSE)
...
pheatmap(mat, scale = "none", 
		annotation_col  = data.frame(colData(gset)[, c("Slide", "Familia", "Grupo2")]),
		annotation_colors =  col_colors, 
		show_rownames = FALSE)
...
1. REPS2 cases, a CHD9 and a case carrier
1. CHD9 and other cases 
1. Controls 

The main conclusions are:

- REPS2 have a distinct methylation pattern from the other individuals. 
...
- The other carriers do not seem to have an affected methylation pattern.
